...
public class FuncionarioServiceDomain : IFuncionarioServiceDomain
{
    ...
    public async Task<ApplicationResult<List<FuncionarioDTO>>> Buscar()
{
    var funcionarios = _context.Funcionarios;

    var resultFuncionarios = await funcionarios
    ...
    var result = ApplicationResult<List<FuncionarioDTO>>.Success(
        resultFuncionarios,
        200,
    ...
    public async Task<ApplicationResult<List<FuncionarioDetalheDTO>>> BuscarTodos()
    {
        var funcionarios = _context.Funcionarios;
        var resultFuncionarios = await funcionarios.Select(f => new FuncionarioDetalheDTO
        {
            // Idfuncionario = f.Idfuncionario,
            // Idusuario = f.Idusuario,
            // Cargo = f.Cargo,
            Especialidade = f.Especialidade,
            NomeUsuario = f.IdusuarioNavigation.Nome,
            Cpf = f.IdusuarioNavigation.Cpf
        }).ToListAsync();

        var result = ApplicationResult<List<FuncionarioDetalheDTO>>.Success(resultFuncionarios, 200, "Registros Localizados.");
