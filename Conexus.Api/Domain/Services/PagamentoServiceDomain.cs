...
public class PagamentoServiceDomain : IPagamentoServiceDomain
{
   ...
   public async Task<ApplicationResult<List<PagamentoDetalheDTO>>> PagamentoNome()
{
    var dados = await _context.Pagamentos
        ...
        {
            NomeUsuario = p.IdalunoNavigation.IdusuarioNavigation.Nome
        })
        .ToListAsync();

    return ApplicationResult<List<PagamentoDetalheDTO>>
        .Success(dados, 200, "Registros localizados.");
}
