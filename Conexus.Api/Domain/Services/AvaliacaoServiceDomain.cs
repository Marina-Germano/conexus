...
public class AvaliacaoServiceDomain : IAvaliacaoServiceDomain
{
    ...
    public async Task<ApplicationResult<List<AvaliacaoDTO>>> Buscar()
    {
...
public async Task<ApplicationResult<List<AvaliacaoDetalheDTO>>> BuscarTodos()
{
    var avaliacoes = await _context.Avaliacaos
        .Include(a => a.IdalunoTurmaNavigation)
        .ThenInclude(at => at.IdalunoNavigation)
        .ThenInclude(al => al.IdusuarioNavigation) // Nome do aluno
        .Include(a => a.IdfuncionarioNavigation)
        .ThenInclude(f => f.IdusuarioNavigation) // Nome do funcionário
        .Include(a => a.IdturmaNavigation) // Nome da turma
        .Select(a => new AvaliacaoDetalheDTO
        {
            Idavaliacao = a.Idavaliacao,
            IdalunoTurma = a.IdalunoTurma,
            Idfuncionario = a.Idfuncionario,
            ...
            NomeUsuario = a.IdalunoTurmaNavigation.IdalunoNavigation.IdusuarioNavigation.Nome,
            NomeFuncionario = a.IdfuncionarioNavigation.IdusuarioNavigation.Nome,
            DescricaoTurma = a.IdturmaNavigation.Descricao
        })
        .ToListAsync();

    return ApplicationResult<List<AvaliacaoDetalheDTO>>.Success(avaliacoes, 200, "Registros Localizados.");
}
