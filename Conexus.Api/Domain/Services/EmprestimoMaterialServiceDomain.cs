...
public class EmprestimoMaterialServiceDomain : IEmprestimoMaterialServiceDomain
{
    ...
    public async Task<ApplicationResult<List<EmprestimoMaterialDTO>>> Buscar()
{
    var emprestimos = _context.EmprestimoMaterials;

    var resultEmprestimos = await emprestimos
        .Select(e => new EmprestimoMaterialDTO
        {
    ...
    var result = ApplicationResult<List<EmprestimoMaterialDTO>>.Success(
        resultEmprestimos,
        200,
    ...
    public async Task<ApplicationResult<List<EmprestimoMaterialDetalheDTO>>> BuscarTodos()
    {
        var emprestimoMaterials = _context.EmprestimoMaterials;
        var resultEmprestimoMaterials = await emprestimoMaterials.Select(e => new EmprestimoMaterialDetalheDTO
        {
            // Idemprestimo = e.Idemprestimo,
            // Idaluno = e.Idaluno,
            // Idmaterial = e.Idmaterial,
            ...
            // Observacoes = e.Observacoes,
            // ValorMulta = e.ValorMulta,
            NomeAluno = e.IdalunoNavigation.IdusuarioNavigation.Nome,
            TituloMaterial = e.IdmaterialNavigation.Titulo
        }).ToListAsync();

        var result = ApplicationResult<List<EmprestimoMaterialDetalheDTO>>.Success(resultEmprestimoMaterials, 200, "Registros Localizados.");
